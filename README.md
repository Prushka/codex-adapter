...
If any upstream response contains the literal `<thinking>` marker, the load balancer logs an error with the provider ID and permanently blocks that provider for the lifetime of the current process. This block is independent of cooldown and prune state and is reported as `blocked` in provider status.
